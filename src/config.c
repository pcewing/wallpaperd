...
typedef wpd_error_t (*sequence_handler)(yaml_document_t *doc, yaml_node_t *node,
                                        struct wpd_config_t *config);
typedef wpd_error_t (*scalar_handler)(yaml_node_t *node,
                                      struct wpd_config_t *config);
...
static wpd_error_t handle_rotation_enabled(yaml_node_t *node,
                                           struct wpd_config_t *config) {
    bool rotation_enabled;
    wpd_error_t error =
        parse_bool((char *)node->data.scalar.value, &rotation_enabled);
    if (error == WPD_ERROR_GLOBAL_SUCCESS)
        config->rotation.enabled = rotation_enabled;
    return error;
}

static wpd_error_t handle_rotation_frequency(yaml_node_t *node,
                                             struct wpd_config_t *config) {
    uint32_t rotation_frequency;
    wpd_error_t error =
        parse_uint32((char *)node->data.scalar.value, &rotation_frequency);
    if (error == WPD_ERROR_GLOBAL_SUCCESS)
...
static wpd_error_t handle_search_paths(yaml_document_t *doc, yaml_node_t *node,
                                       struct wpd_config_t *config) {
...
static wpd_error_t handle_mapping(yaml_document_t *doc, yaml_node_t *node,
                                  const struct map_item_handler *handlers,
                                  struct wpd_config_t *config) {
    ...
    MAP_FOR_EACH(node, itr) {
        ...
        const struct map_item_handler *handler =
            get_handler(handlers, scalar(key));
        if (!handler)
...
wpd_error_t parse_config(const char *path, struct wpd_config_t **config) {
    wpd_error_t error = WPD_ERROR_GLOBAL_SUCCESS;
    yaml_document_t doc;
...
wpd_error_t locate_config(char **config_path) {
