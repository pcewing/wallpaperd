...
void print_error(const xcb_generic_error_t *error) {
    assert(error);
...
void print_geometry(const xcb_get_geometry_reply_t *geometry) {
    assert(geometry);
...
wpd_error_t select_random_wallpaper(const struct wpd_db_t *db, int width,
                                    int height, char **result) {
...
void wpd_get_xcb_image_band_data(const struct wpd_image_t *image, int row_index,
...
xcb_image_t *wpd_create_xcb_image_band(xcb_connection_t *connection,
    ...
    xcb_image_t *xcb_image =
        xcb_image_create(image->width, row_count, XCB_IMAGE_FORMAT_Z_PIXMAP,
                         format->scanline_pad, format->depth,
...
wpd_error_t wpd_xcb_image_put_chunked(xcb_connection_t *connection,
                                      const xcb_setup_t *setup,
                                      xcb_pixmap_t pixmap,
                                      xcb_gcontext_t gcontext,
                                      struct wpd_image_t *image) {
    uint32_t max_request_length_bytes, bytes_per_row, rows_per_request,
        row_index;
    ...
    while (row_index < image->height) {
        ...
        xcb_image_t *xcb_image = wpd_create_xcb_image_band(
            connection, setup, image, row_index, row_count);
...
xcb_pixmap_t wpd_put_image_on_pixmap(xcb_connection_t *connection,
                                     const xcb_setup_t *setup,
    ...
    xcb_generic_error_t *xcb_free_gc_error =
        xcb_request_check(connection, xcb_free_gc_cookie);
...
xcb_atom_t get_atom(xcb_connection_t *connection, const char *atom_name) {
    ...
    xcb_intern_atom_cookie_t cookie = xcb_intern_atom(
        connection, only_if_exists, strlen(atom_name), atom_name);
...
// Note: The structure returned by this method needs to be freed by the caller
xcb_get_property_reply_t *get_pixmap_property(xcb_connection_t *connection,
                                              xcb_window_t window,
                                              xcb_atom_t atom) {
    ...
    xcb_get_property_cookie_t cookie = xcb_get_property(
        connection, delete, window, atom, XCB_ATOM_PIXMAP, offset, length);
...
xcb_pixmap_t get_pixmap_atom(xcb_connection_t *c, const char *atom_name,
                             xcb_window_t window) {
...
void set_pixmap_atom(xcb_connection_t *c, const char *atom_name,
                     xcb_window_t window, xcb_pixmap_t pixmap_id) {
    xcb_atom_t atom = get_atom(c, atom_name);

    xcb_get_property_reply_t *pixmap_property =
        get_pixmap_property(c, window, atom);

    xcb_void_cookie_t change_property_cookie;
    change_property_cookie = xcb_change_property_checked(
        c, XCB_PROP_MODE_REPLACE, window, atom, pixmap_property->type,
        pixmap_property->format, 1, (unsigned char *)&pixmap_id);
...
wpd_error_t wpd_set_wallpaper(xcb_connection_t *connection,
                              const xcb_setup_t *setup,
                              const xcb_screen_t *screen,
                              struct xcb_get_geometry_reply_t *geometry,
                              const char *image_path) {
    struct wpd_image_t *image;
    xcb_pixmap_t new_pixmap, xroot_pixmap, esetroot_pixmap;
    xcb_generic_error_t *error;
    ...
    esetroot_pixmap =
        get_pixmap_atom(connection, "ESETROOT_PMAP_ID", screen->root);

    // Create a new background pixmap
    new_pixmap =
        wpd_put_image_on_pixmap(connection, setup, screen, screen->root, image);
    ...
    xcb_void_cookie_t xcb_free_pixmap_cookie =
        xcb_free_pixmap(connection, xroot_pixmap);
    ...
    xcb_clear_area(connection, 1, screen->root, 0, 0, geometry->width,
                   geometry->height);
...
wpd_error_t wpd_set_wallpaper_for_screen(const struct wpd_db_t *db,
    ...
    xcb_get_geometry_cookie_t cookie =
        xcb_get_geometry(connection, root_window);
    ...
    char *image_path = NULL;
    wpd_error_t wpd_error = select_random_wallpaper(
        db, geometry->width, geometry->height, &image_path);
    if (wpd_error != WPD_ERROR_GLOBAL_SUCCESS) {
        return wpd_error;
    }

    wpd_set_wallpaper(connection, setup, screen, geometry, image_path);
...
wpd_error_t wpd_set_wallpapers(const struct wpd_db_t *db) {
